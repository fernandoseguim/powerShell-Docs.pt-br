...
Para obter mais informações sobre como isso afeta a compatibilidade de script/módulo entre o PowerShell Core e o Windows PowerShell, veja [compatibilidade com versões anteriores com o Windows PowerShell] [#compatibilidade com versões anteriores-com-windows-powershell]
...
- Adicione um arquivo de configuração JSON chamado `PowerShellProperties.json` em `$PSHome` para armazenar algumas configurações necessárias antes do tempo de inicialização (por exemplo, `ExecutionPolicy`).
...
Se você deseja recusar essa telemetria, basta excluir `$PSHome\DELETE_ME_TO_DISABLE_CONSOLEHOST_TELEMETRY`.
A exclusão desse arquivo ignora toda a telemetria mesmo antes da primeira execução do PowerShell.
