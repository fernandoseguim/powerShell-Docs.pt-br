...
A tabela a seguir mostra os elementos da interface do usuário da página do item da Galeria do PowerShell, que são controlados pelo editor. Cada item indica se pode ser controlado pelo manifesto de módulo ou de script.
...
| **Proprietários** | Os proprietários representam a lista de usuários que podem atualizar itens na Galeria do PowerShell. A lista de proprietários não está incluída no manifesto do item. A documentação adicional descreve como [gerenciar proprietários do item](../how-to/publishing-items/managing-item-owners.md). | Não | Não |
